...
class solve:
    ...
    def gen_patterns(self, length: int, clues: List[int]) -> List[List[int]]:
        ...
        def place_with_lead(lead: int):
            ...
            if remain < 0:
                return
            for suffix in self.gen_patterns(remain, rest):
    ...
    def line_consistent(
        self, patterns: List[List[int]], current: List[int]
    ) -> List[List[int]]:
        ok = []
        for p in patterns:
            good = True
    ...
    def deduce_from_patterns(self, patterns: List[List[int]]) -> Optional[List[int]]:
        if not patterns:
            return None
        w = len(patterns[0])
        out = []
    ...
    def apply_updates(self, line: List[int], deduced: List[int]) -> bool:
        changed = False
    ...
    def pretty_io(self, grid: List[List[int]]) -> str:
        mapc = {UNKN: "-", EMPTY: "0", FILL: "1"}
        return "\n".join("".join(mapc[c] for c in row) for row in grid)

    def solve_nonogram_partial(
        self, rows: List[List[int]], cols: List[List[int]]
    ) -> List[List[int]]:
        """
        始终返回结果：
        - 确定: 1
        ...
        row_all = [self.gen_patterns(w, clue) for clue in rows]
        col_all = [self.gen_patterns(h, clue) for clue in cols]
        ...
        def propagate() -> bool:
            ...
            while changed_any:
                ...
                # 行传播
                for r in range(h):
                    row_ok[r] = self.line_consistent(row_ok[r], grid[r])
                    if not row_ok[r]:
                        return False  # 矛盾
                    ded = self.deduce_from_patterns(row_ok[r])
                    if ded is None:
                        return False
                    if self.apply_updates(grid[r], ded):
                        changed_any = True
                # 列传播
                t = self.transpose(grid)
                for c in range(w):
                    col_ok[c] = self.line_consistent(col_ok[c], t[c])
                    if not col_ok[c]:
                        return False  # 矛盾
                    ded = self.deduce_from_patterns(col_ok[c])
                    if ded is None:
                        return False
                    if self.apply_updates(t[c], ded):
                        changed_any = True
                grid[:] = self.transpose(t)
            return True
        ...
        def snapshot():
            return (
                [row[:] for row in grid],
                [list(p) for p in row_ok],
                [list(p) for p in col_ok],
            )
        ...
        def record_partial():
            ...
            if best_partial is None or completeness_score(grid) > completeness_score(
                best_partial
            ):
