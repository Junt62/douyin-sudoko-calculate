...
def get_mouse_pos_screen_top_left():
    # CGEventGetLocation 返回的是全局坐标，原点在左下，y 向上
    loc = Quartz.CGEventGetLocation(Quartz.CGEventCreate(None))
    x_bl = loc.x
    y_bl = loc.y
    H = get_main_screen_height()
    # 转为“屏幕左上为原点，y 向下”
    x_tl = int(round(x_bl))
    y_tl = int(round(y_bl))
    return x_tl, y_tl
...
def get_mouse_pos_in_window(window_bounds):
    mx_tl, my_tl = get_mouse_pos_screen_top_left()
...
def get_window_size(app_name: str = None) -> Optional[Dict]:
...
def get_window_id(app_name: str = None) -> Optional[Dict]:
    windows = CGWindowListCopyWindowInfo(
        kCGWindowListOptionOnScreenOnly, kCGNullWindowID
    )
    for w in windows:
...
def get_window_capture(window_id, window_size):
...
def get_window_capture_foreground(win_size) -> Image:
    with mss.mss() as sct:
        image = sct.grab(
            {
                "left": win_size["X"],
